...
The Helius MCP server should start automatically with this plugin. **Recommended**: If Helius MCP tools are available (e.g., `heliusWallet`, `heliusAsset`, `heliusTransaction`), use them. They handle auth, pagination, and error handling automatically.

If not available, try restarting **Cursor**, or manually add the Helius MCP server via **Settings > Cursor Settings > MCP** with command `npx helius-mcp@latest`. Either way, continue with the task — all workflows below also work via SDK or REST.
...
- For TX submission of copy trades, defer to the `build` skill (Sender + priority fees).
- For DeFi-specific signals (lending positions, perp PnL), defer to the `jupiter` skill.
- For DEX/aggregator-level data (smart-money signals from OKX), defer to the `okx` skill.
...
- OKX — see the `okx` skill for composition patterns
...
### Related skills
- `build` — base Solana primitives, transaction sending
- `okx` — DEX aggregation + OKX smart-money signals
- `jupiter` — DeFi signals (lending, perps, DCA)
